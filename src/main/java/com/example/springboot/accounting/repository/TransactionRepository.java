...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
	...
	/*
	@Query("SELECT new FinancialStatementLine(t.type, SUM(t.amount)) " + "FROM Transaction t "
			+ "WHERE EXTRACT(YEAR FROM t.date) = :year " + "GROUP BY t.type")
	public List<FinancialStatementLine> sumTransactionsByTypeAndYear(@Param("year") Integer year);

	@Query("SELECT new FinancialStatementLine(t.type, SUM(t.amount)) " + "FROM Transaction t "
			+ "WHERE t.date >= :startDate AND t.date <= :endDate " + "GROUP BY t.type")
	public List<FinancialStatementLine> sumTransactionsByTypeAndFiscalYear(@Param("startDate") LocalDate startDate,
			@Param("endDate") LocalDate endDate);

	@Query("SELECT new FinancialStatementLine(t.type, SUM(t.amount)) " + "FROM Transaction t "
			+ "WHERE t.date <= :date " + "GROUP BY t.type")
	public List<FinancialStatementLine> sumTransactionsByTypeAndDate(@Param("date") Date date);

	@Query("SELECT new FinancialStatementLine(t.type, SUM(t.amount)) " + "FROM Transaction t "
			+ "WHERE t.date <= :date " + "GROUP BY t.type")
	public List<FinancialStatementLine> sumTransactionsByTypeAndDate(@Param("date") LocalDate date);

	@Query("SELECT new FinancialStatementLine(t.type, SUM(t.amount)) " + "FROM Transaction t "
			+ "WHERE t.isCashFlow = true AND EXTRACT(YEAR FROM t.date) = :year " + "GROUP BY t.type")
	public List<FinancialStatementLine> sumCashFlowTransactionsByTypeAndYear(@Param("year") Integer year);

	@Query("SELECT new FinancialStatementLine(t.type, SUM(t.amount)) " + "FROM Transaction t "
			+ "WHERE t.isCashFlow = true AND t.date BETWEEN :startDate AND :endDate " + "GROUP BY t.type")
	public List<FinancialStatementLine> sumCashFlowTransactionsByTypeAndFiscalYear(@Param("startDate") Date startDate,
    ...
    */
	...
	@Query("SELECT SUM(t.amount) " 
		    + "FROM Transaction t "
		    + "WHERE t.type IN ('DeptRepayment', 'AssetPurchased') AND t.date BETWEEN :startDate AND :endDate")
		public Double getOtherExpensesForFiscalYear(@Param("startDate") Date startDate, @Param("endDate") Date endDate);
