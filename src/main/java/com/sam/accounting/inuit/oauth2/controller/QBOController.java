...
public class QBOController {
	...
	private static final Logger logger = Logger.getLogger(QBOController.class.getName());
    ...
    public String callQBOCompanyInfo(HttpSession session) {
        ...
        try {
	         ...
	         */
				...
				logger.log(Level.SEVERE,"Error while calling executeQuery :: " + e.getMessage());
				...
				try {
					...
					logger.log(Level.SEVERE,"Error while calling bearer token :: " + e.getMessage());
					return new JSONObject().put("response",failureMsg).toString();
				} catch (FMSException e1) {
					logger.log(Level.SEVERE,"Error while calling company currency :: " + e.getMessage());
					return new JSONObject().put("response",failureMsg).toString();
				}
	            
			} catch (FMSException e) {
				List<Error> list = e.getErrorList();
				list.forEach(error -> logger.log(Level.SEVERE,"Error while calling executeQuery :: " + error.getMessage()));
				return new JSONObject().put("response",failureMsg).toString();
			}
	...
	private String processResponse(String failureMsg, QueryResult queryResult) {
		if (!queryResult.getEntities().isEmpty() && queryResult.getEntities().size() > 0) {
			...
			try {
				...
				logger.log(Level.SEVERE,"Exception while getting company info ", e);
				return new JSONObject().put("response",failureMsg).toString();
			}
