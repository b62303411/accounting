...
import com.sam.accounting.model.dto.ExpensesLine;
...
import com.sam.accounting.repository.BankStatementRepository;
...
public class FinancialStatementService {
	...
	public FinancialStatementService(IncomeStatementService incomeStatementService,
			TransactionRepository transactionRepository, BankStatementRepository bankStatementRepository,
	...
	public List<ReportAvailable> getReportAvailabilities(String accountNumber, Integer year) {
		if (year == null) {
			year = 2023;
		}
		List<BankStatement> bankStatements = getBankStatements(accountNumber, year);
	...
	public List<FinancialStatementLine> getBalanceSheet(Date date) {
		...
		// List<FinancialStatementLine> balanceSheetLines =
		// transactionRepository.sumTransactionsByTypeAndDate(date);
		return new ArrayList<FinancialStatementLine>();
	}

	public List<FinancialStatementLine> getCashFlowStatement(Integer year) {
		// a method that retrieves and sums cash flow related transactions based on
		// their type and date
		// List<FinancialStatementLine> cashFlowStatementLines =
		// transactionRepository.sumCashFlowTransactionsByTypeAndYear(year);
		return new ArrayList<FinancialStatementLine>();
	}
	...
	/**
	 * Income Tax: The amount of tax that the company is liable to pay based on its
	 * profit.
	 */
	...
	/**
	 * Net Income: This is the company's total earnings, calculated by subtracting
	 * income taxes from the pretax income. This is often referred to as "the bottom
	 * line" as it generally appears at the bottom of the income statement.
	 */
